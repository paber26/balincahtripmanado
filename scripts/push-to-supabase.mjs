 ...
 * 
 * Pushes the current default content (including all new fields) to Supabase
 * via the running Nuxt server's /api/save-content endpoint.
 * 
 * Usage: node scripts/push-to-supabase.mjs
 */
...
async function pushToSupabase() {
  console.log("📤 Mengirim data ke Supabase via /api/save-content ...\n");

  try {
    const res = await fetch(`${BASE_URL}/api/save-content`, {
      method: "POST",
    ...
    if (result.success) {
      console.log(`✅ Berhasil disimpan ke: ${result.source.toUpperCase()}`);
      if (result.source === "supabase") {
        console.log("   🗄️  Data tersimpan di tabel Supabase: configurations (key='balincah_content')");
      } else {
        console.log("   📁 Data tersimpan ke file lokal: public/content/content.json");
        console.log("   ⚠️  Supabase mungkin tidak terkoneksi. Cek SUPABASE_URL dan SUPABASE_KEY di .env");
      }
    } else {
    ...
    console.error("   Pastikan server berjalan: npm run dev");
    process.exit(1);
  }
}
