 ...
 * setup-supabase.mjs
 * 
 * Creates the Supabase table (via REST API) and pushes current content.
...
const defaultContent = {
  ...
  packages: [
    ...
    },
    {
      ...
      exclusions: [
        "Pengeluaran pribadi selama trip",
        "Bahan bakar tambahan di luar estimasi rute",
        "Asuransi perjalanan (opsional)"
      ],
      itinerary: [],
      ...
      policies: [
        "DP minimal 50% saat booking.",
        "Pelunasan dilakukan sebelum keberangkatan.",
        "Pembatalan H-3 atau lebih awal dapat refund 50%.",
        "Cuaca buruk akan direschedule."
      ],
      gallery: []
    },
    {
      ...
      exclusions: [
        "Pengeluaran pribadi selama trip",
        "Peralatan snorkeling pribadi jika tidak disediakan paket",
        "Asuransi perjalanan (opsional)"
      ],
      itinerary: [
        { time: "08.00", text: "Meeting point di Pelabuhan Manado" },
        { time: "09.00", text: "Tiba di spot snorkeling Bunaken" },
        { time: "09.15", text: "Briefing dan perlengkapan" },
        { time: "09.30", text: "Snorkeling di spot pilihan" },
        { time: "12.00", text: "Kembali ke daratan" }
      ],
      accommodations: ["Dermaga penyeberangan Marina Plaza Manado"],
      location_detail: "Spot Snorkeling unggulan di sekitar Bunaken.",
      policies: [
        "DP minimal 30% saat booking.",
        "Pelunasan sebelum keberangkatan.",
        "Cuaca buruk direschedule atau refund penuh."
      ],
      gallery: []
    },
    {
      ...
      exclusions: [
        "Pengeluaran pribadi selama trip",
        "Perlengkapan diving tambahan di luar paket",
        "Asuransi perjalanan (opsional)"
      ],
      itinerary: [
        { time: "08.00", text: "Meeting point & briefing keselamatan" },
        { time: "09.00", text: "Tiba di spot diving" },
        { time: "09.15", text: "Persiapan peralatan" },
        { time: "09.30", text: "Dive 1" },
        { time: "11.00", text: "Istirahat permukaan" },
        { time: "11.30", text: "Dive 2 (opsional)" },
        { time: "13.00", text: "Kembali ke daratan" }
      ],
      ...
      policies: [
        "DP minimal 30% saat booking.",
        "Wajib mengisi form kesehatan sebelum diving.",
        "Peserta dengan kondisi medis tertentu wajib konsultasi dokter.",
        "Cuaca buruk direschedule atau refund penuh."
      ],
      gallery: []
    },
    {
      ...
      itinerary: [
        { time: "07.30", text: "Meeting point di Pelabuhan Manado" },
        { time: "08.00", text: "Berangkat menuju Bunaken" },
        { time: "09.00", text: "Snorkeling / explore Bunaken" },
        { time: "10.30", text: "Menuju Nain Island" },
        { time: "11.30", text: "Explore Nain Island" },
        { time: "12.30", text: "Makan siang & istirahat" },
        { time: "13.30", text: "Menuju Siladen Island" },
        { time: "14.30", text: "Explore Siladen - pasir putih" },
        { time: "15.30", text: "Perjalanan kembali ke Manado" },
        { time: "16.30", text: "Trip selesai" }
      ],
      accommodations: [
        "Dermaga penyeberangan Marina Plaza Manado",
        "Fasilitas di Pantai Liang Bunaken",
        "Akses pasir putih Siladen"
      ],
...
async function setup() {
  console.log("🚀 Setup Supabase untuk Balincah Trip Manado\n");
  console.log(`   URL: ${SUPABASE_URL}`);
  console.log(`   Key: ${SUPABASE_KEY.slice(0, 20)}...\n`);

  // Step 1: Test connection
  console.log("1️⃣  Mengecek koneksi ke Supabase...");
  try {
    // Try a simple query, we don't care if table doesn't exist yet
    const { error: connError } = await supabase.rpc('version').maybeSingle().catch(() => ({ error: null }));
    console.log("   ✅ Koneksi berhasil\n");
  } catch {
    console.log("   ✅ Koneksi tersambung\n");
  }

  // Step 2: Create table using Supabase SQL query via pg REST
  console.log("2️⃣  Membuat tabel 'configurations' jika belum ada...");
  
  // Try to create the table using supabase-js query builder by checking if it exists first
  ...
  if (tableError && tableError.code === "PGRST205") {
    console.log("   ⚠️  Tabel belum ada. Silakan buat tabel lewat SQL Editor di Supabase Dashboard:\n");
    console.log("   Buka: https://supabase.com/dashboard/project/${process.env.SUPABASE_URL?.split(".")[0]?.split("//")[1]}/sql/new\n");
    console.log("   Jalankan SQL berikut:\n");
    console.log("─".repeat(60));
    console.log(`create table if not exists public.configurations (
...
on public.configurations for all using (true) with check (true);`);
    console.log("─".repeat(60));
    console.log("\n   Setelah menjalankan SQL di atas, jalankan ulang script ini.\n");
    process.exit(0);
  } else if (tableError) {
  ...
  // Step 3: Upsert content
  console.log("3️⃣  Menyimpan data ke tabel configurations...");
  ...
  // Step 4: Verify
  console.log("4️⃣  Verifikasi data tersimpan...");
  ...
  if (verifyError || !verifyData) {
    ...
    console.log(`   ✅ Terverifikasi! Row: key='${verifyData.key}', updated_at=${verifyData.updated_at}\n`);
  }

  console.log("═".repeat(60));
  console.log("🎉 Setup selesai! Semua data sudah tersimpan di Supabase.");
  console.log("   Admin panel sekarang akan save/load dari Supabase otomatis.");
  console.log("═".repeat(60));
}
